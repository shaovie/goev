...
func (c *Conn) onUpgrade(buf []byte) bool {
    ...
    for bufOffset < bufLen {
        ...
        pos = bytes.IndexByte(buf[bufOffset:], CRLF)
        if pos > 0 {
            sepP := bytes.Index(buf[bufOffset:pos], ':')
            if sepP < 0 {
                return false // Abnormal connection. close it
            }
            var header [2]string
            header[0] = string(buf[bufOffset:bufOffset+sepP])
            header[1] = string(buf[bufOffset+sepP+1:bufOffset+pos])
    ...
    for i := range headers {
        ...
        if strings.EqualFold(header[0], "Connection") &&
            ...
            host = strings.TrimSpace(header[1]);
            ...
            header[1] == "13" {
            okVersion = true
        } else if strings.EqualFold(header[0], "Sec-WebSocket-Extension") {
            ...
            for j := range arr {
                if s := strings.TrimSpace(j); len(s) > 0 {
...
func (c *Conn) onFrame(buf []byte) bool {
    ...
    for bufLen > 0 {
        ...
        if wsf.hlen == 0 { // partial header
            c.partialFrameTime = unix.Time().UnixMilli()
            c.partialBuf = append(c.partialBuf, buf...)
            break
        }
        ...
        var payloadBuf []byte
        if wsf.payload > 0 {
            if bufLen-wsf.hlen < wsf.payload { // partial payload
                c.partialFrameTime = unix.Time().UnixMilli()
                c.partialBuf = append(c.partialBuf, buf...)
                break
            }
            payloadBuf = buf[wsf.hlen:wsf.hlen+wsf.payload]
        }

        bufOffset += wsf.hlen + wsf.payload
        bufLen -= wsf.hlen + wsf.payload
        ...
        if wsf.isfin {
            ...
            if wsf.opcode != FrameContinue { // first continue frame
                c.wsFrame = wsf
                c.continueFrame.start = true
                c.continueFrame.buf = make([]byte, 0, len(payloadBuf))
                c.continueFrame.buf = append(c.continueFrame.buf, payloadBuf)
            } else { // other continue frame 
                if c.continueFrame.start == false {
                    return false // except
                }
                c.continueFrame.buf = append(c.continueFrame.buf, payloadBuf)
            }
...
func (c *Conn) parseFrameHeader(buf []byte) (frameHeader, bool) {
    var fh wsFrame
    bufLen := len(buf)
    if bufLen < 2 {
...
func (c *Conn) writeControlFrame(opcode int, data []byte) {
    ...
    buf := make([]byte, maxControlFramePayloadSize + 2)
...
func (c *Conn) OnCloseFrame(c CloseInfo) {
}
func main() {
